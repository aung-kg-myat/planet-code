<template>
  <div
    ref="refElement"
    class="container-style duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] container relative isolate w-[250px] transition-transform will-change-transform [aspect-ratio:17/21] [contain:layout_style] [perspective:600px]"
    @pointermove="handlePointerMove"
    @pointerenter="handlePointerEnter"
    @pointerleave="handlePointerLeave"
  >
    <div
      class="rounded-t-[10px] duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] grid h-full origin-center overflow-hidden rounded-lg border border-slate-800 transition-transform will-change-transform [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] hover:filter-none hover:[--duration:200ms] hover:[--easing:linear] hover:[--opacity:0.6]"
    >
      <div
...
.background-style {
  ...
  --rainbow: repeating-linear-gradient(
      0deg,
      rgb(207, 27, 21) calc(var(--step) * 1),
      rgb(116, 105, 26) calc(var(--step) * 2),
      rgb(15, 23, 8) calc(var(--step) * 3),
      rgba(131, 255, 247, 0.8) calc(var(--step) * 4),
      rgb(14, 21, 46) calc(var(--step) * 5),
      rgb(158, 0, 220) calc(var(--step) * 6),
      rgb(146, 141, 141) calc(var(--step) * 7)
    )
  ...
  --diagonal: repeating-linear-gradient(
      ...
      #0e152e 0%,
      hsl(180, 10%, 20%) 3.8%,
      /* Darkened even more */ hsl(180, 10%, 20%) 4.5%,
      hsl(180, 10%, 20%) 5.2%,
      #0e152e 10%,
      #0e152e 12%
    )
  ...
  --shade: radial-gradient(
      ...
      /* Almost invisible glare */ rgba(255, 255, 255, 0.05) 40%,
      /* Smooth transition */ rgba(255, 255, 255, 0.08) 100%
        /* Very light fade-out */
    )
    var(--bg-x) var(--bg-y) / 300% no-repeat;
